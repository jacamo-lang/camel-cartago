...
#### Examples
Let's say you want to integrate a semaphor as an artifact to your MAS. The semaphor has an operation of request closure, for pedestrians to cross the street, and it signals drivers and pedestrians when it changes its state. Also, drivers are able to look and perceive at any time the semaphor's light colour. In that case the artifact should have an observable property, lightColour, an operation, requestClosure, and emmits two signals, turnedGreen and turnedRed.
